...
export const CategoryScreen = ({navigation, route}) => {
    ...
    let categoryId = route.params.item.id

    const loadApi = useCallback(async () => {
        LogBox.ignoreLogs(['VirtualizedLists should never be nested']);
        fetch(`https://pack-trade.com/application/category-direction?id=${categoryId}`)
                            ...
                            <View>
                                <FlatList
                                    ...
                                    renderItem={({ item }) => (
                                        <TouchableOpacity
                                            onPress={
                                                () => navigation.navigate('ProductsListScreenStack', {item})
                                            }
